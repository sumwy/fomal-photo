...
def process_image(image_data: str, options: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # PIL 이미지를 OpenCV로 변환
        logger.debug("PIL 이미지를 OpenCV로 변환")
        ...
        # 처리 효율성을 위해 크기 조정
        logger.debug("처리를 위한 이미지 크기 조정")
        image_cv, scale = resize_for_processing(image_cv)
        logger.debug(f"크기 조정 후: {image_cv.shape[1]}x{image_cv.shape[0]}, 스케일: {scale}")
        ...
        logger.debug(f"최종 이미지 크기: {image_pil.size}, 인코딩 크기: {len(encoded_image)}")
        ...
        logger.error(f"이미지 처리 중 오류 발생: {str(e)}", exc_info=True)  # 스택 트레이스 추가
